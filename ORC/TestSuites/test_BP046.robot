...
Documentation  Hiring manager initiates the requisition using position (End to End External candidate)
...            Prerequisite:  Not applicable
...            Environment Specific Data:  Login User,Recruiter User
...            Reusable Data: Requisition ID,Expiry date,Email,Interview Schedule,Interview Date2,Start time,End time,Meeting Duration,Web Conference link,Interviewer Name,Notification Action,Action on Offer,Decline Reason
...            Dynamic Data: Expiry date; Hiring Manager; HRBP; Collaborator; Recruiter
...
*** Variables ***
${json_path}    ./TestData/td_BP046.json
${csv_path}  ./CSV/td_BP046.csv
*** Test Cases ***

Scenario: End To End ( External Candidate )
    ...
    Log  Offer approval(hiring manager)
    Login Using  ${data}[Login User]
    Search for Notifications E2E  ${applicant_name}
    Review Offer and Take Action E2E  ${data}[Notification Action]
    Logout

    Log  Offer extension(recruiter)
    Login Using  ${data}[Recruiter Username]
    click on homepage
    Go to my client group hiring
    Search for requisition id  ${req_id}
    ...
    Log  Candidate accepts the offer
    Authentication for Job Offer E2E  ${data}[Action on Offer]  ${applicant_name}  ${data}[Decline Reason]
    Close Window
