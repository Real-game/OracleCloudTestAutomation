...
Documentation  Current term Expiry Date cannot be earlier than Original Contract Start Date for for Property Contract
...
Test Template  Validate the error message of Current term Expiry Date cannot be earlier than Original Contract Start Date for for Property Contract
...
*** Test Cases ***
Current term Expiry Date cannot be earlier than Original Contract Start Date for for Property Contract with - ${Id} ${login_user} ${property_workspace_name} ${MX_cre_file_number} ${contract_type} ${contract_name} ${contract_rent_structure} ${status} ${tenant_entity_type} ${landlord_entity_type}
    [Tags]  UnifierNegativeTest  ReadOnly

*** Keywords ***
Validate the error message of Current term Expiry Date cannot be earlier than Original Contract Start Date for for Property Contract
    ...
    Validate the error message   Contract Date Validation 2 : Current term Expiry Date cannot be earlier than Original Contract Start Date