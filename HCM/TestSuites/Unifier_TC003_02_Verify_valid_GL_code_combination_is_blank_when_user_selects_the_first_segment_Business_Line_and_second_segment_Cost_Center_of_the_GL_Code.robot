*** Settings ***
Library  DataDriver  ../CSV/td_TC003_02_Verify_valid_GL_code_combination_is_blank_when_user_selects_the_first_segment_Business_Line_and_second_segment_Cost_Center_of_the_GL_Code.csv
...
Documentation  Verify valid GL code combination is blank when user selects the first segment (Business Line) and second segment (Cost Center) of the GL Code
...
Test Template  Verify valid GL code combination is blank when user selects the first segment (Business Line) and second segment (Cost Center) of the GL Code


*** Variables ***
${json_path}    ./TestData/td_TC003_02_Verify_valid_GL_code_combination_is_blank_when_user_selects_the_first_segment_Business_Line_and_second_segment_Cost_Center_of_the_GL_Code.json
${csv_path}  ./CSV/td_TC003_02_Verify_valid_GL_code_combination_is_blank_when_user_selects_the_first_segment_Business_Line_and_second_segment_Cost_Center_of_the_GL_Code.csv

*** Test Cases ***
TC003.02 - Verify valid GL code combination is blank when user selects the first segment (Business Line) and second segment (Cost Center) of the GL Code with - ${Id} ${login_user} ${property_workspace_name} ${MX_cre_file_number} ${contract_type} ${contract_name} ${contract_rent_structure} ${status} ${tenant_entity_type} ${landlord_entity_type} ${CRE_process_rent_value} ${tax_classification_value} ${Business_Line} ${Cost_Center}
    [Tags]  PropertyContract  ModifyData

*** Keywords ***
Verify valid GL code combination is blank when user selects the first segment (Business Line) and second segment (Cost Center) of the GL Code
