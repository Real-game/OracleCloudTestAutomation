...
*** Variables ***
#${json_path}    ./TestData/td_TC016_Validate_the_Flex_code_for_an_employee_with_Dependents.json
#${csv_path}  ./CSV/td_TC016_Validate_the_Flex_code_for_an_employee_with_Dependents.csv
...
#    ${data}=  readJson  ${json_path}
    ...
    Click on Enrollment
    Log  Step 6-7
    Search and click on newly hired Person  ${common_data}[Employee Number]
    Log  Step 8
    Click on People to Cover
    Log  Step 9
    ${depenedent_spouse}  Fetch Dependent  Spouse
    ${depenedent_child}  Fetch Dependent  Child
    click on homepage
    Log  Step 5 - 7
    click on Nevigator
    Select On Tools
    ...
    Select Text Frame
    Verify text format
    Verify Dependent Presence  ${depenedent_spouse}  ${depenedent_child}