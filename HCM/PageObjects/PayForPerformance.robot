...
Apply Filter for Eligible Employees
    ...
    Wait Until Page Contains Element  ${eligibility_status_drop_down}  40s  Eligibility status drop down is not displayed
...
Apply Filter for Ineligible Employees
    Click Element  ${filter_icon}
    Wait Until Page Contains Element  ${eligibility_status_drop_down}  40s  Eligibility status drop down is not displayed
    Click Element  ${eligibility_status_drop_down}
    Wait Until Page Contains Element  ${ineligible_check_box}  20s  Ineligible check box is not displayed
    Click Element  ${ineligible_check_box}
    Capture Page Screenshot
    Sleep  3s
    Click Element  ${go_icon}

Extract Ineligible person number and person name
    Wait Until Page Does Not Contain Element  ${eligible_employee}  40s  Eligible employees are also listed
    Scroll Element Into View  ${columns_hidden_text}
    ${person_number_value}=  Get Element Attribute  ${first_employee_person_number}  innerHTML
    ${person_name_value}=  Get Element Attribute  ${first_employee_name}  innerHTML
    ${person_rating_value}=  Get Element Attribute  ${first_employee_rating}  innerHTML
